...
Plans:
- [x] 03-01-PLAN.md — Wave 0 test stubs + Migration 012 (telemetry_logs RLS auth.jwt()->>'sub' fix + get_latest_telemetry RPC) + VehicleService resource() refactor replacing loadVehicles/loadInitialTelemetry with effect() bridge [FLEET-01, FLEET-02]
- [x] 03-02-PLAN.md — FleetMapComponent stale state (4-state getVehicleState, initialFitDone guard, tooltip) + Dashboard/VehicleGrid loading/empty states + ShellComponent connection pill + toast wiring [FLEET-01, FLEET-02, FLEET-03]
